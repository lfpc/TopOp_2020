...
if outeriter < 0.5
  [f0val,df0dx,fval,dfdx] = toy2(xval,model,index_p,len_U,constraint);
  innerit=0;
...
while ((kktnorm > kkttol)||outit<minoutit) && outit < maxoutit
  outit   = outit+1;
  outeriter = outeriter+1;
  
  if beta_jump
      beta = (floor(outit/beta_jump)+1)*(beta_i);
...
%%%% The results should be put in f0valnew and fvalnew.
  [f0valnew,fvalnew] = toy1(xmma,model,index_p,len_U,constraint);
...
%%%% The residual vector of the KKT conditions is calculated:
  ...
  outvector1 = [outeriter innerit xval'];
  outvector2 = [f0val 0.0064289/(f0val) fval' outeriter kktnorm]
  kktnorm
  if live_plot
      mphplot(model,'pg3')
      set(gcf,'Visible','on')
